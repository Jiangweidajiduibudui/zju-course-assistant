...
const taskInstructions: Record<keyof typeof llmTasks, string> = {
  ...
  generateTimetable:
    "Select exactly one supplied candidate per target course, preserving the locked baseline and all confirmed hard constraints. Explain the selections and tradeoffs in concise Chinese, even if course or teacher names use other languages. Do not claim all conflicts are resolved: deterministic validation is performed separately.",
