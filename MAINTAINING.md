...
A prior runtime archive may supply only `runtime/`, `node_modules/` and `browsers/` when its full archive checksum is verified and its Node version, direct dependencies and lockfile hash match the new source exactly. Always rebuild and replace the application, refresh release metadata/notices/manifests, and repeat target-platform and relocation checks. Do not carry forward user data or a previous application build.
