...
test("Demo mainline 入口：同意后加载合成数据并进入主链路", async ({ page }) => {
  ...
  await expect(
    page.getByText("用户前置已完成；生成推荐暂不可用：LLM/key 未配置，且等待 selection-model 接入"),
  ).toBeVisible();
  await expect(page.getByText("学分上限已填写：18 学分")).toBeVisible();
  await expect(page.getByText("待选池已准备：3 门课程 / 5 个候选教学班")).toBeVisible();
  await expect(page.getByText("LLM/key 配置：尚未配置 LLM key 或完成能力检测")).toBeVisible();
  await expect(
    page.getByText("当前不收集、不保存 key，不会调用 LLM、planner 或写入 zdbk"),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "生成推荐（LLM 未配置）" })).toBeDisabled();
